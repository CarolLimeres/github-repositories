...
export default class Main extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    try {
      ...
      if (repoExists) throw new Error("This repository was already saved");
    ...
    return (
      <Container>
        ...
        {repositories.length > 0 ? (
          <List>
            {repositories.map((repository) => (
              <li key={repository.name}>
                <div>
                  <FaTimes
                    onClick={() => this.handleRepoRemove(repository.name)}
                  />
          ...
          <EmptyList>
            You haven&apos;t saved any repositories yet. Try searching above
            or&nbsp;
